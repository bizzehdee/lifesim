...
public static class GenomeMutator
{
    public static Genome Mutate(Genome genome, MutationConfig config, TraitBounds bounds, Prng mutationStream)
    {
        ...
        return new Genome
        {
            ...
            LearningDecay = Drift(genome.LearningDecay, bounds.LearningDecay, config, mutationStream),
            Sexuality = Drift(genome.Sexuality, bounds.Sexuality, config, mutationStream),
