...
**Status: scaffolded, awaiting manual execution.** This spike is the go/no-go for using Avalonia-for-web as the browser demo surface. It cannot be validated in a headless CI/agent environment — it requires a human running a browser and observing framerate, bundle size, and cold start.
...
## What to prototype
A throwaway scene (not production code) that draws N moving markers on a custom-drawn Avalonia control, mirroring the eventual organism map:
...
- Browser: `dotnet publish src/LifeSim.App.Browser -c Release -o artifacts/wasm`, then serve `artifacts/wasm/wwwroot` over HTTP (WASM threading needs COOP/COEP headers) and open in a browser.
