...
**Spec section:** §21 (source of truth). Built as the **aggregate single-tile body** model (decision below), so all effects are pure functions of the genome and the deterministic core needed no spatial refactor.

**Settled design decisions (via user, this session):** aggregate single-tile body (not spatial multi-tile); six cell types (Germ/Feeder/Store/Defender/Mover/Sensor); capacity driven by Store cells; body mass = `cell_count × Size` so a 1-cell body is exactly a plain organism (continuous transition); per-world toggle, default on. Controller = the existing single NEAT brain (the body is one agent); no separate developmental program.

- [x] **Body model & genome (§21):** evolvable `cell_count` + six specialisation weights on `Genome`; `Morphology` (pure) turns genome + config into every derived quantity. Founders start at `cell_count = 1` (generalist); the traits mutate/inherit via the existing `GenomeMutator` in fixed order (§9).
- [x] **Cell types & division of labour:** six jobs, each a bonus for emphasis *above* the ⅙ generalist baseline so a generalist body is neutral — Germ (reproduction), Feeder (grazing yield), Store (capacity), Defender (combat mass + thermal resist), Mover (locomotion), Sensor (effective acuity). A body has `fraction × N` cells per job (many per type).
- [x] **Germline / soma split (§8):** reproduction is gated on germ fraction (`germ_reproduction_threshold`); a germ-poor body is sterile soma. Offspring inherit the mutated body-plan genome.
- [x] **Square-cube economy (§3, §21):** maintenance scales with volume (base metabolism × N + per-cell coordination cost) while grazing intake is capped by surface (`∝ N^⅔`); the rising per-cell deficit caps viable size emergently. Store cells raise the per-body energy ceiling (`Organism.EnergyCapacity`); bigger bodies cost more but can store more.
- [x] **Controller:** the existing single NEAT brain drives the whole body; sensing/actions unchanged except reproduction-readiness now reflects mass-scaled cost + fertility, and Sensor cells sharpen the acuity used for noise injection.
- [x] **Snapshot schema (§12):** seven new genome fields round-trip; capacity is recomputed from genome + config on load (not stored) so save/reload stays byte-identical. Schema/config bumped to 1.2.
- [x] **Determinism (§9):** all body math is pure and deterministic; the new trait mutations draw from the mutation stream in fixed order. Both flagship determinism tests + the eventful-determinism test stay green.
- [x] **Metrics (§14):** mean `cell_count` on `TraitAverages` + a `cell_count` histogram + `avg_cell_count` CSV column.
- [x] **UI (§18):** setup-screen **Multicellularity** toggle; inspector "Body" section (cell count, per-type composition bars, fertile / sterile-soma status).
- [x] **Calibration (§15):** the default multicellular world persists without runaway or extinction, mean body size stays well below the hard cap under the square-cube law, and all Phase 12 calibration scenarios still pass with multicellularity live.

**Exit criteria:** Organisms evolve differentiated multi-cell bodies with a germline/soma split, selected and reproduced as whole individuals; the square-cube law bounds body size; capacity/specialisation trade-offs are real; the model round-trips through the snapshot and replays deterministically; bodies are inspectable and toggleable. ✅ 270 tests pass (Core 176 / Console 16 / Determinism 14 / App 55 / Calibration 9); `dotnet format --verify-no-changes` clean.

**Judgment calls worth flagging:**
- **Aggregate (single-tile) body, not spatial multi-tile** — chosen with the user to keep the deterministic core intact and deliver the requested mechanics (square-cube cap, specialisation, capacity trade-off) without a cell-per-tile grid refactor. A body's cells are a continuous composition, not placed tiles; there is no developmental program.
- **Specialisation is relative to the ⅙ generalist baseline**, so a generalist 1-cell body is byte-identical to the pre-§21 organism — the transition is genuinely continuous and the toggle-off path reduces exactly to the old engine.
- **Size is capped emergently by the square-cube law**, not a hard limit — volume upkeep (∝ N) outruns surface-limited intake (∝ N^⅔), so mean body size settles well under the `cell_count` bound under selection.
- **Fuller cell-type-ratio metrics (per-type population means) were not added** — only mean/distribution of `cell_count`; per-body composition is visible in the inspector. A candidate follow-up.
