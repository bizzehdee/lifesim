...
- A **day/night and seasonal cycle** runs over the whole world (a fast daily rhythm with a slow yearly one on top of it). It swings **temperature** — nights and winters are colder — and sets the **light** on every tile (open biomes are brightest; shaded ones like swamp get less). With **photosynthesis** on (the default), ground energy regrows in step with the light: fast in daytime and summer, stalled at night and in winter. So food is not just a place but a *time*, and following the light pays. (Turn photosynthesis off and food regrows at a steady biome rate regardless of the cycle.)
...
- Traits include body **size**, **speed**, a **thermal comfort band** (centre ± width), **sensing radii** and **acuity**, **metabolic efficiency** (rule 3), the **defences** armour / evasion / toxicity (rule 6), neural **plasticity** and **learning decay** for within-life learning (rule 5), **generosity** (rule 9), a **sexuality** leaning (rule 7), and a multicellular **body plan** (rule 9). Several of these are *evolved-in* capabilities — the defences, learning, and sexuality all start at **zero** and must be earned by selection; the rest start varied.
...
- **Sensing** gives each organism a snapshot of its surroundings — local biome, temperature, its own energy, nearby organisms (and how *related* they are), reproductive readiness, and a global stress signal. It also senses the **light** at its tile, the **time of day and season** (as smooth cyclic signals, so a brain can *anticipate* dawn or winter, not just react), and the **direction of the brightest ground nearby** — the raw material for phototaxis (sun-seeking or shade-seeking) and daily/seasonal rhythms. Low acuity blurs these readings with noise.
...
- **Brains can also learn within a single life.** Beyond evolving across generations, an organism can adjust its own brain's connection weights as it lives, nudged by whether its recent actions gained or lost energy (reward-modulated plasticity). How *much* it learns (**plasticity**) and how fast learned changes fade back toward the inherited baseline (**learning decay**) are themselves evolvable traits, starting at zero. Learning is **not inherited** — offspring inherit the evolved (germline) brain, not what a parent learned — but the *capacity* to learn is, so lineages can evolve to learn well and, over time, turn hard-won behaviour into instinct (the **Baldwin effect**). A plastic brain costs extra upkeep, so learning only pays off where the world is variable enough to reward it.
...
Seed, map size, starting population, and any tuning constant — plus toggles for **cooperation**, **senescence**, **multicellularity**, and **photosynthesis**, and the number of **threads** the engine uses (which never changes the result, only the speed).
