...
- **Reproduction cost** sheds its size penalty: a fully-differentiated body buds at nearly single-cell cost instead of whole-body-mass cost.

So a lopsided (single-specialist) or generalist body is still hard-capped small by the square-cube law, but a *well-differentiated* body escapes it and can grow large — specialised multicellularity, not mere size, is what selection rewards. (This makes big bodies K-strategists: fewer, larger, cheaper-per-cell individuals rather than many single cells.)
...
Capacity is a deterministic function of genome + config, recomputed on load (not serialized), so save/reload stays byte-identical. Reproduction is gated on germ fraction and costs energy proportional to whole-body mass, so bigger bodies are dearer to bud; offspring inherit the body-plan genome (cell count + six weights) with mutation, exactly like every other trait — new draws come from the mutation stream in fixed order (§9). **Multicellularity is heritable with an upward bias:** a multicellular parent nudges its offspring's cell count up by `offspring_growth_bias · (parent_cells − 1)` after mutation, so the trait reinforces itself rather than eroding back to unicellular under symmetric drift near the floor — and it is the square-cube economy, not drift, that caps the eventual size. The nudge draws no randomness, so replay stays deterministic. All effects are pure functions of the genome (see the engine's `Morphology`), so the whole model is deterministic and replayable. Metrics track mean and distribution of `cell_count`; the inspector shows a body's cell composition and whether it is fertile or sterile soma.
