...
/// <summary>
/// The live set of active environmental event modifiers. Owns the Environment
...
/// sensor value fed to every brain.
/// </summary>
public sealed class EnvironmentState
{
    ...
    /// <summary>True while any Resource Blight is active — ground energy regeneration is suspended.</summary>
    public bool BlightActive => HasActive(EventType.ResourceBlight);

    /// <summary>True while any Density-Dependent Plague is active — crowded organisms take an extra drain.</summary>
    public bool PlagueActive => HasActive(EventType.DensityPlague);

    /// <summary>Sum of every active Climatic Anomaly's signed temperature shift, added to tile temperature.</summary>
    public double TemperatureOffset =>
        _modifiers.Where(m => m.Type == EventType.ClimaticAnomaly).Sum(m => m.Magnitude);

    /// <summary>
    /// Global Stress Level sensor: graded by how many event types are active,
    ...
    /// <summary>
    /// The Environment phase: first age active modifiers and drop expired ones,
    /// then roll for newly-triggered events against the events PRNG stream in a fixed order —
    /// blight, plague, anomaly. At most one modifier of each type is active at
